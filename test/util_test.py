...
class UtilTest(unittest.TestCase):
    ...
    def test_find_non_substr(self):
        self.assertEqual(util.find_non_substr('crea'), 'a')
        self.assertEqual(util.find_non_substr('creaian'), 'b')
        self.assertEqual(util.find_non_substr('creaian bob'), 'c')
        self.assertEqual(util.find_non_substr('creaian bob can'), 'd')
        # skip 'e' because 'crea' contains 'e'
        self.assertEqual(util.find_non_substr('creaian bob can do'), 'f')
        self.assertEqual(util.find_non_substr('creaian bob can do fun'), 'g')
        self.assertEqual(util.find_non_substr('creaian bob can do fun things'), 'j')

    def test_iterate_operations_from(self):
        backend = SteemRemoteBackend(nodes=["https://node1.creary.net"], appbase=True)
        cread = SteemInterface(backend)
        result = util.iterate_operations_from(cread, True, 1102, 1103, set())
        expected_op = {
            'type': 'pow_operation',
            'value': {
                'worker_account': 'creary11',
                'block_id': '0000044df0f062c0504a8e37288a371ada63a1c7',
                'nonce': 33097,
                'work': {
                    'worker': 'CREA65wH1LZ7BfSHcK69SShnqCAH5xdoSZpGkUjmzHJ5GCuxEK9V5G',
        ...
        for op in result:
            ...
            self.assertEqual(op['value']['props']['cbd_interest_rate'], expected_op['value']['props']['cbd_interest_rate'])
