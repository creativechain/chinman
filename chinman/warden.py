...
def main(argv):
    ...
    parser.add_argument("-s", "--server", default="http://127.0.0.1:8090", dest="server", metavar="URL", help="Specify cread server to watch over")
    ...
    config = cread.database_api.get_config(x=None)
    ...
    witness_schedule = cread.database_api.get_witness_schedule(x=None)
    witnesses = witness_schedule["current_shuffled_witnesses"]
    scheduled_witnesses = witness_schedule["num_scheduled_witnesses"]
    
    if scheduled_witnesses == config["CREA_MAX_WITNESSES"]:
        print("[√] scheduled witnesses: %s" % config["CREA_MAX_WITNESSES"])
        passfail.append(PREFLIGHT_GO)
    else:
